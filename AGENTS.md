...
src/og.ts      the 1200×630 social card and README banner (og.html[?banner], dev only); `bun run og` renders public/og.png and docs/banner.png
...
bun run og         # re-render the social card and README banner (needs Chrome or Edge; commit the PNGs)
```
